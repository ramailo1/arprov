...
for ($i = 0; $i -lt $lines.Length; $i++) {
    if ($startIdx1 -eq -1 -and $lines[$i].Contains("private val VIDEO_EXTENSIONS")) { $startIdx1 = $i }
    if ($startIdx1 -ne -1 -and $endIdx1 -eq -1 -and $lines[$i].Contains("private suspend fun resolveHost()")) { $endIdx1 = $i }
    if ($startIdx2 -eq -1 -and $lines[$i].Contains("private suspend fun extractM3u8ViaWebView(")) {
        if ($lines[$i-1].Contains("@SuppressLint")) {
            $startIdx2 = $i - 1
        } else {
            $startIdx2 = $i
        }
    }
    if ($startIdx2 -ne -1 -and $endIdx2 -eq -1 -and $lines[$i].Contains("override val mainPage = mainPageOf(")) { $endIdx2 = $i }
}

if ($startIdx1 -ne -1 -and $endIdx1 -ne -1 -and $startIdx2 -ne -1 -and $endIdx2 -ne -1) {
    $newLines = @()
    for ($i = 0; $i -lt $startIdx1; $i++) { $newLines += $lines[$i] }
    $newLines += $newChunk1
    for ($i = $endIdx1; $i -lt $startIdx2; $i++) { $newLines += $lines[$i] }
    $newLines += $newChunk2
    for ($i = $endIdx2; $i -lt $lines.Length; $i++) { $newLines += $lines[$i] }
    
    [System.IO.File]::WriteAllLines($filePath, $newLines, [System.Text.Encoding]::UTF8)
    Write-Host "File updated successfully."
} else {
    Write-Host "Indices not found: $startIdx1, $endIdx1, $startIdx2, $endIdx2"
}
