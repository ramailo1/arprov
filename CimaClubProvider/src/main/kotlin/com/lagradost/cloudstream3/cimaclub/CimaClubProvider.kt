...
class CimaClubProvider : MainAPI() {
    ...
    private fun detectTvType(category: String, url: String): TvType {
        return when {
            category.contains("انمي") -> TvType.Anime
    ...
    override suspend fun getMainPage(
        ...
        val url = when {
            ...
            baseUrl == mainUrl -> {
                if (page == 1) baseUrl else "$mainUrl/page/$page/"
            }

            // 🎬 Movies pagination
            baseUrl.contains("/movies") -> {
                if (page == 1) baseUrl else "$baseUrl/page/$page/"
            }

            // 📺 Series / Anime pagination (OFFSET)
            baseUrl.contains("/series") || baseUrl.contains("/anime") -> {
                if (page == 1) baseUrl else "$baseUrl?offset=$page"
            }
    ...
    private fun Element.toSearchResponse(): SearchResponse? {
        ...
        val rawTitle = selectFirst("inner--title h2")?.text()
            ?: return null
        ...
        val type = detectTvType(category, link)
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        val title = cleanTitle(
            doc.selectFirst("h1")?.text() ?: return null
        )

        val poster = doc.selectFirst(".Poster img, .image img")
            ?.attr("src")

        val description =
            doc.selectFirst(".Story, .StoryArea")?.text()

        val category =
            doc.selectFirst(".category")?.text() ?: ""

        val episodeElements =
            doc.select(".Episodes-List .Small--Box, .BlocksHolder .Small--Box")

        val isSeries =
            episodeElements.isNotEmpty() || url.contains("/series/")
    ...
    override suspend fun loadLinks(
        ...
        // 4️⃣ download links → direct video
